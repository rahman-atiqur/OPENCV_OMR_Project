...
# cv2.imshow("Tempalte Image", refImage)
...
def layoutDesign():
    ...
    # def comando():
    #    messagebox.showinfo("What user choose", n.get())
    # Button(frame5, text="Select", width=20, height=2, command= comando).place(x=100,y=100)

    def combo_choose(value):
        if value != "":
            # messagebox.showinfo("Choice", "You have selected: " + value)
            image = cv2.imread(image_path)
            # image = cv2.imread('OMR_100_skewed.jpg')
            angle, corrected = correct_skew(image)
            # print('Skew angle:', angle)
            # img=cv2.imshow('De-skewed', corrected)
            cv2.imwrite('de-skewed.jpg',corrected)
            # cv2.imwrite('template'+str(angle)+'.jpg',corrected)

            ###################################
            template_file=utils.timerCount("de-skewed.jpg")
            ###################################
            ...
            # messagebox.showinfo("Hello", "YHello")
